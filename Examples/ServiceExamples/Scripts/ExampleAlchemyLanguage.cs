...
public class ExampleAlchemyLanguage : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    ////  Get Author URL POST
    //if (!alchemyAPI.GetAuthors(OnGetAuthors, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get authors URL POST!");

    ////Get Author HTML POST
    //if (!alchemyAPI.GetAuthors(OnGetAuthors, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get authors HTML POST!");

    ////Get Concepts Text POST
    if (!alchemyAPI.GetRankedConcepts(OnGetConcepts, exampleText_watsonJeopardy))
      Log.Debug("ExampleAlchemyLanguage", "Failed to get concepts Text POST!");

    ////Get Concepts HTML POST
    //if (!alchemyAPI.GetRankedConcepts(OnGetConcepts, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get concepts HTML POST!");

    ////Get Concepts URL POST
    //if (!alchemyAPI.GetRankedConcepts(OnGetCzoncepts, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get concepts HTML POST!");

    ////Get Date URL POST
    //if (!alchemyAPI.GetDates(OnGetDates, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get dates by URL POST");

    ////Get Date Text POST
    //if (!alchemyAPI.GetDates(OnGetDates, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get dates by text POST");

    ////Get Date HTML POST
    //if (!alchemyAPI.GetDates(OnGetDates, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get dates by HTML POST");

    ////Get Emotions URL POST
    //if (!alchemyAPI.GetEmotions(OnGetEmotions, exampleURL_watsonJeopardy, true))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get emotions by URL POST");

    ////Get Emotions Text POST
    //if (!alchemyAPI.GetEmotions(OnGetEmotions, exampleText_watsonJeopardy, true))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get emotions by text POST");

    ////Get Emotions HTML POST
    //if (!alchemyAPI.GetEmotions(OnGetEmotions, watson_beats_jeopardy_html, true))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get emotions by HTML POST");

    ////Extract Entities URL POST
    //if (!alchemyAPI.ExtractEntities(OnExtractEntities, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get entities by URL POST");

    ////Extract Entities Text POST
    //if (!alchemyAPI.ExtractEntities(OnExtractEntities, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get entities by text POST");

    ////Extract Entities HTML POST
    //if (!alchemyAPI.ExtractEntities(OnExtractEntities, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get entities by HTML POST");

    ////Detect Feeds URL POST
    //if (!alchemyAPI.DetectFeeds(OnDetectFeeds, "http://time.com/newsfeed/"))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get feeds by URL POST");

    ////Detect Feeds HTML POST
    ////if (!alchemyAPI.DetectFeeds(OnDetectFeeds, ycombinator_html))
    ////    Log.Debug("ExampleAlchemyLanguage", "Failed to get feeds by URL POST");

    ////Extract Keywords URL POST
    //if (!alchemyAPI.ExtractKeywords(OnExtractKeywords, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get keywords by URL POST");

    ////Extract Keywords Text POST
    //if (!alchemyAPI.ExtractKeywords(OnExtractKeywords, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get keywords by text POST");

    ////Extract Keywords HTML POST
    //if (!alchemyAPI.ExtractKeywords(OnExtractKeywords, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get keywords by HTML POST");

    ////Extract Languages URL POST
    //if (!alchemyAPI.GetLanguages(OnGetLanguages, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get languages by text POST");

    ////Extract Languages Text POST
    //if (!alchemyAPI.GetLanguages(OnGetLanguages, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get languages by text POST");

    ////Extract Languages HTML POST
    //if (!alchemyAPI.GetLanguages(OnGetLanguages, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get languages by HTML POST");

    ////Get Microformats URL POST
    //if (!alchemyAPI.GetMicroformats(OnGetMicroformats, exampleURL_microformats))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get microformats by text POST");

    ////Get Microformats HTML POST
    ////if (!alchemyAPI.GetMicroformats(OnGetMicroformats, microformats_html))
    ////    Log.Debug("ExampleAlchemyLanguage", "Failed to get microformats by text POST");

    ////Get PublicationDate URL POST
    //if (!alchemyAPI.GetPublicationDate(OnGetPublicationDate, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get publication dates by url POST");

    ////Get PublicationDate HTML POST
    //if (!alchemyAPI.GetPublicationDate(OnGetPublicationDate, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get publication dates by html POST");

    ////Get Relations URL POST
    //if (!alchemyAPI.GetRelations(OnGetRelations, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get relations by text POST");

    ////Get Relations Text POST
    //if (!alchemyAPI.GetRelations(OnGetRelations, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get relations by text POST");

    ////Get Relations HTML POST
    //if (!alchemyAPI.GetRelations(OnGetRelations, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get relations by HTML POST");

    ////Get Sentiment URL POST
    //if (!alchemyAPI.GetTextSentiment(OnGetTextSentiment, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get sentiment by text POST");

    ////Get Sentiment Text POST
    //if (!alchemyAPI.GetTextSentiment(OnGetTextSentiment, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get sentiment by text POST");

    ////Get Sentiment HTML POST
    //if (!alchemyAPI.GetTextSentiment(OnGetTextSentiment, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get sentiment by HTML POST");

    ////Get Targeted Sentiment URL POST
    //if (!alchemyAPI.GetTargetedSentiment(OnGetTargetedSentiment, exampleURL_watsonJeopardy, "Jeopardy|Jennings|Watson"))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get targeted sentiment by text POST");

    ////Get Targeted Sentiment Text POST
    //if (!alchemyAPI.GetTargetedSentiment(OnGetTargetedSentiment, exampleText_watsonJeopardy, "Jeopardy|Jennings|Watson"))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get targeted sentiment by text POST");

    ////Get Targeted Sentiment HTML POST
    //if (!alchemyAPI.GetTargetedSentiment(OnGetTargetedSentiment, watson_beats_jeopardy_html, "Jeopardy|Jennings|Watson"))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get targeted sentiment by HTML POST");

    ////Get Taxonomy URL POST
    //if (!alchemyAPI.GetRankedTaxonomy(OnGetRankedTaxonomy, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get ranked taxonomy by text POST");

    ////Get Taxonomy Text POST
    //if (!alchemyAPI.GetRankedTaxonomy(OnGetRankedTaxonomy, exampleText_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get ranked taxonomy by text POST");

    ////Get Taxonomy HTML POST
    //if (!alchemyAPI.GetRankedTaxonomy(OnGetRankedTaxonomy, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get ranked taxonomy by HTML POST");

    ////Get Text HTML POST
    //if (!alchemyAPI.GetText(OnGetText, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get text by text POST");

    ////Get Text URL POST
    //if (!alchemyAPI.GetText(OnGetText, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get text by text POST");

    ////Get Raw Text HTML POST
    //if (!alchemyAPI.GetRawText(OnGetText, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get raw text by text POST");

    ////Get Raw Text URL POST
    //if (!alchemyAPI.GetRawText(OnGetText, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get raw text by text POST");

    ////Get Title HTML POST
    //if (!alchemyAPI.GetTitle(OnGetTitle, watson_beats_jeopardy_html))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get title by text POST");

    ////Get Title URL POST
    //if (!alchemyAPI.GetTitle(OnGetTitle, exampleURL_watsonJeopardy))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get title by text POST");

    ////  Get Combined Data URL POST
    //if (!alchemyAPI.GetCombinedData(OnGetCombinedData, exampleURL_watsonJeopardy, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get combined data by text POST");

    ////Get Combined Data Text POST
    //if (!alchemyAPI.GetCombinedData(OnGetCombinedData, exampleText_watsonJeopardy, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true))
    //    Log.Debug("ExampleAlchemyLanguage", "Failed to get combined data by text POST");

    ////Get Combined Data HTML POST
    //if (!alchemyAPI.GetCombinedData(OnGetCombinedData, watson_beats_jeopardy_html, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true))
